...
const SERVER_STALE_TIMEOUT_SECS: i32 = 5;
...
pub async fn create_job(
    ...
    let manifest_path = format!("{}.manifest", gcs_file_path);

    sqlx::query(
        r#"
        INSERT INTO model_deployment_jobs (deployment_job_id, gcs_file_path, gcs_manifest_path, total_shards, total_size, shard_size, status, created_at, updated_at)
...
pub async fn get_all_jobs(pool: &DbPool) -> Result<Vec<DeploymentJob>, sqlx::Error> {
    let rows = sqlx::query_as::<_, (String, String, String, String, i32, i64, i64, String)>(
        r#"
        SELECT deployment_job_id, created_at, gcs_file_path, gcs_manifest_path, total_shards, total_size, shard_size, status
    ...
    Ok(rows
        .into_iter()
        .filter_map(
            |(deployment_job_id, created_at, gcs_file_path, gcs_manifest_path, total_shards, total_size, shard_size, status)| {
...
pub async fn get_job_server_progress(
    ...
    let rows = sqlx::query_as::<_, (String, i32, Option<i32>, Option<f64>, Option<i64>, String, String)>(
        r#"
        SELECT t.server_address, t.last_shard_id_completed, t.current_shard_id, t.current_shard_progress_pct, t.throughput_bps, t.status, COALESCE(s.status, 'unhealthy') as server_status
    ...
    Ok(rows
        .into_iter()
        .map(|(server_address, last_shard_id_completed, current_shard_id, current_shard_progress_pct, throughput_bps, status, server_status)| ServerTaskProgress {
            server_address,
            ...
            upstream: None,
            throughput_bps: throughput_bps.map(|t| t as u64),
            current_shard_id,
...
pub async fn get_server_tasks(
    ...
    let rows = sqlx::query_as::<_, (String, String, String, i32, i64)>(
        r#"
        SELECT j.deployment_job_id, j.gcs_file_path, j.gcs_manifest_path, j.total_shards, j.shard_size
        FROM server_model_deployment_tasks t
        JOIN model_deployment_jobs j ON t.deployment_job_id = j.deployment_job_id
        WHERE t.server_address = $1 AND t.status != 'completed'
    ...
    Ok(rows
        .into_iter()
        .filter_map(|(deployment_job_id, gcs_file_path, gcs_manifest_path, total_shards, shard_size)| {
            let deployment_job_id = Uuid::parse_str(&deployment_job_id).ok()?;
            Some(DeploymentTask {
                deployment_job_id,
                gcs_file_path,
...
pub async fn update_task_progress(
    ...
    .bind(status)
    .bind(throughput_bps.map(|t| t as i64))
    .bind(server_address)
    .bind(job_id.to_string())
    .execute(pool)
    .await?;
